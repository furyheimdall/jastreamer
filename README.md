...
- **Optional desktop:** a Windows 10/11 x64 portable ZIP or Linux amd64 DEB that discovers a Server or accepts its HTTP(S) address and displays its hosted Web interface. Browser audio remains the default. Compatible Windows builds add opt-in WASAPI shared/exclusive output and Windows media controls, still using the Server queue; see [Windows audio](INSTRUCTION.md#windows-audio). There is no independent saved-music player or ARM64 desktop package; Linux arm64 clients can use a browser.
