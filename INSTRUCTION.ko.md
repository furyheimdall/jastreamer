...
**Settings → 재생 및 파일 검사 이력**에서 Server에 저장되는 공용 이력을 확인하세요. 결과 종류나 렌더러로 필터링할 수 있습니다. 렌더러 보고에는 렌더러 이름·ID, 곡, 오류 코드, 제공된 재생 위치와 펼쳐 볼 수 있는 구조화 진단이 표시됩니다. 이전 로그에서 가져온 기록은 이름 없이 렌더러 ID만 표시될 수 있습니다. 최근 5,000건을 보관하며 이력을 보는 동작은 재생을 변경하지 않습니다.
