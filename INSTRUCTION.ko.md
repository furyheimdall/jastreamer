...
jastreamer는 Linux 또는 네이티브 Windows에서 Server를 실행합니다. 두 대상 모두 내장 Web 화면과 UPnP/DLNA 출력을 제공하며 Google Cast를 선택적으로 켤 수 있습니다. AirPlay 전송은 Linux 컨테이너 패키지에서만 사용할 수 있습니다. 선택 사항인 Desktop·네이티브 모바일·PWA는 Server에 접속합니다. **이 기기**는 같은 Server 대기열에 로컬 출력을 추가하며 브라우저·Desktop·iOS에서는 브라우저 오디오, 네이티브 Android 앱에서는 Media3 서비스를 사용합니다. 요구 사항, 패키지 검증, 설치와 업데이트·롤백은 [한국어 설치 및 업데이트 안내서](INSTALL.ko.md)를 따르세요.
...
Kotlin 앱은 공유 Web 화면 바깥에 서버 선택과 네이티브 로컬 재생을 추가합니다. [호환되는 Server/Web UI 버전, 설치와 APK 업데이트 규칙](INSTALL.ko.md#android)은 PWA 설치와 별도입니다.

- 발견된 서버나 최근 서버를 고르거나 HTTP(S) 루트 주소를 입력하고 **확인 후 연결**을 누르세요. 새로 실행하면 자동으로 연결하지 않습니다. 최근 항목도 다시 확인하며 검색 이름은 HTTP 식별자 검증 뒤에만 사용합니다.
- 네이티브 헤더에는 선택한 Server와 포트를 포함한 origin이 표시됩니다. **서버**는 네트워크 출력의 재생을 정지하지 않고 서버 선택으로 돌아갑니다. 저장된 항목의 Server UUID가 달라지면 기존 세션을 조용히 재사용하지 않고 거부합니다.
- Android의 시스템 바·키보드 여백은 네이티브 셸에서 한 번만 적용하고 Web 화면에서 같은 safe area 여백을 다시 더하지 않습니다.
...
- 네이티브 셸은 화면 닫기, 백그라운드 전환, 서버 변경이나 다시 열기만으로 Play·Stop을 보내지 않습니다. 대기열·현재 곡·다음 곡 결정은 Server가 담당합니다. 다른 Server의 화면을 보는 것만으로 기존 네이티브 출력이 정지하지 않으며, 다른 Server에 로컬 출력을 등록하려면 기존 로컬 재생부터 정지해야 합니다.
- 휴대전화에서 들으려면 재생을 정지하고 **Android · jastreamer (이 기기) [로컬 오디오]** 또는 저장한 별칭을 직접 선택한 뒤 **재생**을 누르세요. Media3 포그라운드 서비스가 인증된 same-origin HTTP(S) GET/Range로 음원을 받아 운영체제가 선택한 스피커·헤드폰·Bluetooth 경로로 재생합니다. 지원 형식은 Media3·Android 디코더에 따라 다르며 독점 모드나 bit-perfect를 보장하지 않습니다.
- Android 시스템 미디어 카드와 잠금 화면의 재생·일시 정지·정지·이전·다음·지원되는 탐색은 Server에 명령을 보내며 Android에 별도 대기열을 만들지 않습니다. 네이티브 재생은 WebView의 백그라운드 전환이나 Activity·WebView 재생성과 독립적으로 유지됩니다. 같은 앱은 재생성 뒤 자신이 소유한 정확한 등록에 다시 연결하며 다른 브라우저·기기에 **(이 기기)** 표시를 넘기지 않습니다.
- 일시적 음원 오류는 현재 곡·위치와 가장 최근의 재생·일시 정지 의도를 유지하며 12초 복구 구간 내에서 최대 세 번 재시도합니다. 정지·곡 교체·인증 상실·등록 lease 만료는 복구를 취소합니다. 영구 오류는 무한 재시도하지 않고 표시합니다. 화면의 Server 연결 재검증도 제한적으로 재시도하며 TLS·Server 식별자 오류는 즉시 종료합니다.
- 프로세스 종료나 등록 상실·만료 시 로컬 재생은 정지합니다. 프로세스 재시작이나 등록의 최종 상실 뒤 자동 재등록·재생은 하지 않습니다. 오류를 해결하고 필요하면 다시 로그인한 뒤 Server 재생을 정지하고 로컬 출력을 직접 선택해 재생하세요. Android·제조사의 프로세스·배터리 제한은 여전히 적용되며 오프라인 재생이나 service worker 명령 대기열은 없습니다.
- 확인된 Server의 현재 최상위 문서에만 로컬 출력 연결·상태·이름 변경용 제한된 네이티브 인터페이스를 제공합니다. 쿠키·등록 자격 증명·임의 네이티브 호출·파일시스템 접근은 노출하지 않습니다. 다른 클라이언트는 브라우저 오디오를 유지하며 iOS에 이 인터페이스를 추가하지 않습니다.
...
| 이 기기에서 소리가 없거나 연결이 끊김 | 표시되면 **재생 허용** 선택, 브라우저·OS 음소거와 출력 경로 확인, 재생 담당 페이지 유지; 새로고침이나 등록 만료 뒤에는 필요하면 정지하고 **이 기기**를 다시 선택해 직접 재생; 디코더·통신 오류 보존 |
...
Android 재생 오류 진단에는 대응하는 Server와 APK가 필요합니다. [업데이트 절차](INSTALL.ko.md#upgrade)에 따라 호환 Server를 먼저 업데이트한 뒤 해당 APK를 설치하세요. 실패한 네이티브 명령과 최종 재생 오류 보고에는 Media3 오류 코드·이름, 발생 시각, 재생 위치, 원인 예외 타입과 길이가 제한된 코드 심볼 기반 호출 스택이 포함됩니다. 확인 가능한 HTTP 상태와 코덱·오디오 출력의 숫자 오류 코드도 함께 남깁니다. 예외 메시지 원문, 미디어 URL, 파일 경로와 인증정보는 제외합니다. Server가 받은 보고는 별도 ADB 수집 없이 기존 순환 로그에 자동 보관합니다. 오프라인 업로드 대기열은 아니므로 전달 전에 프로세스가 종료되거나 Server 연결이 끊기면 수집되지 않을 수 있으며, 이전에 유실된 예외를 복원하지는 못합니다.
