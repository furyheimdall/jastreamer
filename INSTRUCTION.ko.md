...
- 헤더에는 선택한 Server와 포트를 포함한 완전한 origin이 표시됩니다. **서버 변경**은 재생을 멈추지 않고 서버 선택으로 돌아갑니다. 백그라운드에서 돌아오면 페이지를 보여주기 전에 Server UUID를 다시 확인하며 식별자·네트워크 오류에서는 이전 페이지와 키보드에 접근할 수 없습니다.
...
- 종료, 백그라운드 전환과 서버 변경은 Play·Stop을 보내지 않습니다. 로컬 Renderer, 오프라인 재생이나 명령 cache는 없습니다. 외부 탐색, 새 창, 다운로드, 파일 선택기와 네이티브 미디어·기기 권한 요청을 차단하며 파일 업로드가 필요한 작업은 브라우저를 사용하세요.
