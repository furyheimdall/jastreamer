...
2. **중단 전에 다운로드:** 승인된 레지스트리 주소가 있다면 기존 Docker 인증이나 비공개 대화형 인증으로 정확한 대상 다이제스트를 내려받습니다. 토큰을 대화나 설정 파일에 넣지 마세요. 전달된 멀티아키텍처 이미지에서는 호스트에 맞는 아키텍처가 자동 선택되며, `amd64` 또는 `arm64`인지 확인합니다. 오프라인 패키지는 2절의 검증·가져오기 절차를 따르세요. 가변 `latest` 태그를 사용하거나 레지스트리 주소를 지어내거나 이전 이미지를 삭제하지 마세요.
...
전체 이미지를 내려받아 검증해. 안전한 레지스트리 인증이나 문서의
오프라인 가져오기를 사용하고 비밀정보를 대화·로그에 기록하지 마.
