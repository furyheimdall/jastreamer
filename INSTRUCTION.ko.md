...
jastreamer는 Linux 또는 네이티브 Windows에서 Server를 실행합니다. 두 대상 모두 내장 Web 화면과 UPnP/DLNA 출력을 제공하며 Google Cast를 선택적으로 켤 수 있습니다. AirPlay 전송은 Linux 컨테이너 패키지에서만 사용할 수 있습니다. 선택 사항인 Desktop·네이티브 모바일·PWA는 Server에 접속합니다. **이 기기**는 독립 네이티브 오디오 엔진 없이 같은 Server 대기열에 브라우저 오디오 출력을 추가합니다. 요구 사항, 패키지 검증, 설치와 업데이트·롤백은 [한국어 설치 및 업데이트 안내서](INSTALL.ko.md)를 따르세요.
...
1. 재생을 정지하고 **재생 기기**에서 **(이 기기)**가 붙은 출력(예: **Windows · Chrome (이 기기) [웹 브라우저]**)을 선택하세요. 휴대전화에서는 간결한 플레이어를 펼치면 선택기가 나타납니다.
...
기본 이름은 페이지에서 확인할 수 있는 OS·브라우저 정보이며, 컴퓨터의 호스트 이름이나 사용자가 휴대전화에 지정한 이름을 읽은 값은 아닙니다. 출력 선택기 옆 연필 버튼 **이 브라우저 출력 이름 변경**에서 **작업실 PC** 같은 별칭을 저장하세요. **기본 이름 사용**을 누르거나 별칭을 비워 저장하면 자동 이름으로 돌아갑니다. 이름은 UTF-8 기준 80바이트까지이며 한글은 글자당 여러 바이트를 사용합니다.
...
재생을 담당하는 페이지를 열어 두세요. 해당 페이지를 닫거나 새로고침하면 로컬 출력이 해제되며, 등록의 연결 유지가 끊기면 대기열을 버리지 않고 출력을 사용 불가로 처리합니다. 필요하면 정지한 뒤 **이 기기**를 다시 선택하고 직접 재생하세요. 다른 Control을 닫아도 재생 담당 브라우저는 정지하지 않으며 네트워크 출력은 Control 종료와 무관하게 동작합니다. 모바일 백그라운드·잠금 화면 재생은 브라우저·WebView와 운영체제에 따라 달라 보장하지 않으며, 오프라인 재생이나 네이티브 백그라운드 오디오 서비스는 없습니다.
...
- 네이티브 셸은 앱 종료, 백그라운드 전환, 서버 변경이나 다시 열기에서 Play·Stop을 보내지 않습니다. 재생과 대기열은 Server가 관리합니다. Web 페이지가 **이 기기** 출력을 맡고 있다면 해당 페이지나 등록의 연결 유지가 끊길 때 앞서 설명한 브라우저 출력이 해제됩니다. 독립 네이티브 오디오 엔진, 오프라인 재생, service worker 명령 대기열이나 네이티브 JavaScript bridge는 없습니다.
