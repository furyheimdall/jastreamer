...
func (service *server) browse(kind string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if raw := values.Get("recursive"); raw != "" {
			...
			if err != nil {
				writeError(w, fault.New(http.StatusBadRequest, "INVALID_QUERY", "하위 폴더 옵션이 올바르지 않습니다."))
				return
			}
		}
		page, err := service.options.Library.Browse(r.Context(), library.Query{Kind: kind, Search: values.Get("q"), AlbumID: values.Get("album_id"), Artist: values.Get("artist"), Genre: values.Get("genre"), RootID: values.Get("root_id"), Path: values.Get("path"), Recursive: recursive, Liked: liked, Played: played, Sort: values.Get("sort"), Offset: offset, Limit: limit})
		if err != nil {
			writeError(w, err)
			return
		}
