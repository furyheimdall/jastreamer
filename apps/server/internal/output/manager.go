...
func validBackend(backend Backend) bool {
	return backend.Controller != nil && (backend.Protocol == ProtocolUPnP || backend.Protocol == ProtocolAirPlay || backend.Protocol == ProtocolCast || backend.Protocol == ProtocolBrowser)
}
