...
test("workflow is protected, pinned, and does not request OIDC", () => {
  ...
  const buildMsi = readFileSync(`${root}build-msi.ps1`, "utf8");
  const sign = readFileSync(`${root}sign.ps1`, "utf8");
  ...
  expect(buildMsi).toContain('-d "ProductVersion=$Version"');
  expect(buildMsi).toContain('-d "SourceExe=$exe"');
  expect(buildMsi).toContain("$LASTEXITCODE");
  expect(sign).toContain("$LASTEXITCODE");
  expect(sign).toContain("SignerCertificate");
});
