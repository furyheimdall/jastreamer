...
export const verifySupplyChain = (context, artifacts, tools) => {
  ...
  for (const [index, artifact] of artifacts.entries()) {
    ...
    if (!exactKeys(definition, ["buildType", "externalParameters", "resolvedDependencies"]) || !exactKeys(definition?.externalParameters, ["artifactKind", "platforms"]) || !exactKeys(attestation.statement.predicate?.runDetails, ["builder"]) || !exactKeys(dependencies?.[0], ["uri", "digest"]) || !exactKeys(dependencies?.[0]?.digest, ["gitCommit"]) || definition?.buildType !== context.trust.config.materialPolicy.buildType || definition.externalParameters?.artifactKind !== artifact.kind || JSON.stringify(definition.externalParameters?.platforms) !== JSON.stringify(expectedPlatforms) || dependencies?.length !== 1 || dependencies[0]?.uri !== context.trust.config.materialPolicy.sourceUri || dependencies[0]?.digest?.gitCommit !== context.receipt.source.revision || !context.trust.config.builders.includes(builder)) return tools.denied("SLSA_INVALID", "provenance");
