...
export type ArtifactManifest = { readonly component: "server" | "control" | "renderer"; readonly version: string; readonly tag: string; readonly artifacts: readonly string[]; readonly license: "Apache-2.0"; readonly third_party_notices: string; readonly source_revision: string; readonly distribution?: "ci-artifact-only"; readonly previous_component_tag?: string; readonly previous_repository_tag?: string; readonly changelog_start?: string; readonly mutable_full_tag?: boolean; readonly signing: Signing; readonly windows_signing?: Signing };
const semver = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/;
const prefixes = { server: "server-v", control: "control-v", renderer: "renderer-v" } as const;
const required = { server: ["jastreamer-server_{v}_windows_amd64.msi", "jastreamer-server_{v}_linux_amd64.deb", "jastreamer-server_{v}_linux_amd64.rpm", "jastreamer-server_{v}_linux_arm64.deb", "jastreamer-server_{v}_linux_arm64.rpm"], control: ["jastreamer-control_{v}_web.zip", "jastreamer-control_{v}_windows.msix", "jastreamer-control_{v}_android_universal.apk"], renderer: ["jastreamer-renderer_{v}_windows_amd64.msi", "jastreamer-renderer_{v}_diagnostic.zip"] } as const;
export function validateRelease(path: string): ValidationResult {
  ...
  for (const file of files) { let item: ArtifactManifest; try { item = JSON.parse(readFileSync(file, "utf8")) as ArtifactManifest; } catch { errors.push("MALFORMED_MANIFEST"); continue; } manifests.push(item); const expected = prefixes[item.component];
    ...
    const win = item.windows_signing ?? item.signing; if (item.component === "server" || item.component === "control" || item.component === "renderer") { if (win.ca) errors.push("CA_CAPABLE_CERTIFICATE"); if (!win.fingerprint) errors.push("MISSING_CERTIFICATE_FINGERPRINT"); if (win.code_signing_eku !== true) errors.push("WRONG_CODE_SIGNING_EKU"); if (win.publisher !== "CN=jastreamer") errors.push("PUBLISHER_MISMATCH"); if (win.private_key) errors.push("PRIVATE_KEY_PRESENT"); }
    if (item.component === "control" && (item.signing.application_id !== "io.jastreamer.control" || !item.signing.keystore_lineage || item.signing.private_key !== false)) errors.push("ANDROID_SIGNING_LINEAGE_INVALID");
    if (item.component === "renderer" && item.distribution !== "ci-artifact-only") errors.push("RENDERER_PUBLICATION_DISABLED");
  }
  for (const manifest of manifests) { const previousRepositoryTag = manifest.previous_repository_tag; if (previousRepositoryTag && !manifests.some((sibling) => sibling.component !== manifest.component && previousRepositoryTag.startsWith(prefixes[sibling.component]))) errors.push("PREVIOUS_REPOSITORY_TAG_INVALID"); }
