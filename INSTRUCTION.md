...
**Bit-perfect badge.** While an Exclusive-mode path is actually active, a **Bit-perfect** badge sits next to the track information in the player bar; the Android USB direct path uses the same badge. It reads **Bit-perfect** when the application path qualifies, and **Not bit-perfect** with the reason when the exclusive path is active but the path was altered. It is hidden for browser audio, network renderers, and shared or system output. Selecting it opens the matching audio settings panel; it never changes playback.
...
**USB bit-perfect audio (direct USB)**
...
All this needs is a connected USB Audio Class device and USB permission for it. The panel names whatever is missing, and normal playback is unaffected while the option is off.

**Widening is the only automatic conversion.** A source narrower than the DAC's container is zero-padded into the wider container — 16-bit into 24- or 32-bit, 24-bit into 32-bit — so every original bit survives and the path stays bit-transparent; the panel reports valid bits and container bits separately. A mono source on a stereo-only DAC is duplicated to both channels: it plays, but it is reported as not bit-transparent.

**Nothing else is converted.** Resampling, bit-depth reduction, downmixing, and float-to-integer conversion never happen inside the app. **When the USB device cannot take the format** decides what happens instead:

- **Skip track** (default) fails that track with the specific reason — unsupported sample rate, channel count, precision, or encoding — and the Server continues under its normal terminal-failure policy.
- **Play through Android output** hands the DAC back to Android for that track. The panel then shows **Not bit-perfect** with the reason, and the next compatible track claims the DAC again.

**Reading the panel.** It separates the **Requested path** — USB device name, USB direct — from the **Actual active path**: engine state, sample rate, valid and container bits, channels, synchronisation type, feedback rate, and underruns. The actual values come from the USB stream that is really running, not from the request.
...
| A UPnP/DLNA renderer starts and then stops immediately | The renderer fetched media from an address the Server never saw during discovery, so the grant was refused with `403` and `error_category="source_mismatch"` | A renderer with several addresses on one interface is supported: the Server serves media to every address discovery observed for it (the SSDP sender, the LOCATION host, its control URLs, and any address that answers a direct search sent to the addresses already known for it). If the log still shows a source mismatch, the request came from an unobserved address — check `remote_class` and `allowed_addresses` in `<data_dir>/logs/server.log*`, keep the renderer on the discovered interface, and rescan outputs after changing its network configuration |
...
| **USB bit-perfect (direct USB)** shows no device, or a track fails on the USB path | No USB Audio Class device is attached, USB permission was not granted, or the DAC cannot take that track's sample rate, channel count, precision, or encoding | Reconnect the dongle and accept the permission request the phone shows when the option is switched on; the panel names the missing prerequisite and the reason a track failed. The app never resamples, reduces bit depth, downmixes, or converts float to integer to make a track fit — set **When the USB device cannot take the format** to **Play through Android output** to hear such tracks, or keep **Skip track** |
