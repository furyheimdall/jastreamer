...
- **Saved music** opens the app's local **Library / Playlists / Queue / Settings** without Server HTML, network access, or login. Browse or search saved tracks, albums, artists, and folders from any source Server. A completed import includes local metadata and artwork and remains available after logout, session expiry, account loss, Server/profile removal, or an older or unavailable Server.
- A compatible Server/Web UI with the versioned v1 download capability can show separate **Download** controls for tracks, albums, and playlists. Confirm the destination and either **Original** (the verified source bytes) or **Space saving (AAC-LC 256 kbps in M4A)** when the Server advertises it. Downloading never starts playback or changes the shared Server queue. A playlist import is a one-time snapshot that preserves order and duplicate entries; it does not synchronize later Server edits.
- **Downloads** shows preparation, transfer, verification, local import, partial failure, pause/resume, cancellation, and destination controls. The default policy waits for an unmetered network; enable **Allow metered or mobile data** explicitly to use one. Pending work remains bound to its originating Server and signed-in principal. A Server without the v1 capability cannot start a new import, but it can still be controlled normally and cannot affect music already saved.
- Create, rename, reorder, and delete local playlists without changing Server playlists. Removing a playlist entry or deleting a local playlist leaves its audio file in place. The local queue preserves order and repeated occurrences, is independent of the Server queue, and supports move, remove, seek, previous/next, shuffle, and repeat.
