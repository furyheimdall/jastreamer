...
같은 application ID와 같은 서명의 APK를 덮어쓰면 앱 전용 저장 음원, 복사한 앨범 아트, 로컬 재생목록, 기기 대기열·위치, 폴더, 다운로드·언어 환경설정, 최근 Server와 격리 Server 프로필을 보존합니다. **최근 서버 목록에서 제거**는 바로가기만 지우며 로그아웃, 프로필 제거 또는 저장된 음악 삭제가 아닙니다. 세션을 끝내고 해당 Server의 미완료 가져오기를 중지하려면 Server 화면에서 로그아웃하세요. 세션/프로필을 제거해도 완료된 기기 소유 음악을 회수하거나 삭제하지 않습니다.
