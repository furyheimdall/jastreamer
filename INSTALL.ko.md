...
일반 Linux의 기본 프로젝트 위치는 대상 사용자의 `~/.config/jstreamer`이며 config와 data는 각각 `~/.config/jstreamer/config`, `~/.config/jstreamer/data`로 분리합니다. 대상 호스트의 실제 홈 경로를 확인하고 sudo나 `/srv` 쓰기 권한을 가정하지 마세요. Synology는 승인된 영구 프로젝트·공유 폴더 경로를 유지하며 일반적으로 `/volume1/docker/jastreamer` 아래 config와 data를 분리합니다. 프로젝트/config/data는 승인된 음악 루트 밖에 둡니다. 기존 설치는 별도의 이동 승인과 검증한 백업 없이 옮기지 않습니다.
...
무설치 업데이트 전에 실제 launcher와 설정을 확인해 `data_dir`와 모든 음악 루트의 위치를 파악하세요. 외부 드라이브, UNC 공유, 심볼릭 링크·junction의 실제 대상도 포함하며 설치 폴더 옆 `data`·`music`이라고 가정하지 않습니다. 실제 경로와 권한을 기록하고 외부 위치도 백업하거나 해당 음악의 기존 백업·스냅샷을 확인·검증하세요. 경로나 백업 범위를 알 수 없으면 중단합니다. 아래의 설치 폴더 백업만으로 외부 루트까지 보호되지는 않습니다.

기존 무설치 설치를 업데이트할 때는 Ctrl+C로 종료한 상태에서 `server.json`, `data`, `music`을 포함한 전체 폴더를 백업하세요. 새 ZIP을 검증해 별도 임시 폴더에 푼 다음 프로그램, launcher, 템플릿, 포함된 `samples`, 고지, 라이선스와 빌드 정보를 비롯한 패키지 소유 파일만 기존 설치에서 교체합니다. `server.json`을 교체하거나 `data`·`music`을 삭제·이동하지 마세요. 업데이트에 샘플을 추가하는 일은 아래의 별도 명시적 선택 절차이며, 기존 상태에 최초 실행 샘플 복사를 적용하면 안 됩니다. 변경하지 않은 설정, URL, 계정, 보관함, 앨범 아트, 대기열, 플레이리스트와 세션을 재시작 뒤 확인하고 이전 검증 패키지와 일치하는 백업을 보관하세요.
...
4. **확인한 경로의 일관된 백업:** Server가 멈춘 상태에서 실제 config/data 폴더 전체, 존재하는 SQLite WAL/SHM 파일과 저장된 Compose·환경 파일을 백업하고 이전 이미지 식별자·원래 경로를 기록합니다. 모든 실제 음악 루트도 백업하거나 외부 루트까지 포함하는 현재의 기존 백업·스냅샷을 식별하고 검증하세요. 읽기 전용 마운트는 백업이 아닙니다. 백업은 원본 트리 밖에 두고 파일 메타데이터를 보존하며, 내용·checksum과 분리된 위치에서의 복원·읽기 가능 여부를 검증하고 포함·제외 범위를 정확히 기록합니다. 범위가 불명확하거나 검증되지 않았으면 교체를 중단하고 제외된 음악을 백업 완료라고 보고하지 마세요. 계정·세션·설정·자격 증명이 들어갈 수 있으므로 비공개 데이터로 보호하며 백업을 위해 원본 음악을 덮어쓰거나 수정하지 않습니다.
